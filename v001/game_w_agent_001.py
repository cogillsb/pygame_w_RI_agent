...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self, name, start_pos, health_pos, speed, screen, world, agent=None):
        # Call the parent class (Sprite) constructor
        super().__init__()
        self.world = world
        self.name = name
    ...
    def ai(self):
        ...
        if self.agent:
            #Get the action from world agent
            if self.action > 7:
...
class World():
    ...
    def round_reset(self):
        if self.round>1:
            ...
            img, r = self.draw_text(f'{winner} WINS') 
            r.center = self.screen.get_rect().center 
            r.y -= 100         
            self.screen.blit(img, r)
            pygame.display.update()
        ...
        for k, v in PLAYER_DAT.items():
            p = Player(k, v[0], v[1], 5, self.screen, self)
            if p.name == 'K': p.agent = True
            self.players.add(p)
        ...
        self.draw_text(f'ROUND {self.round}')  
        pygame.display.update()
        #Pause for 5 secs
    ...
    def run_agent(self):
        action = self.world.agent.act(cur_state)            
